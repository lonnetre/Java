...
public class VSAuctionServer {
    public static void main(String[] args) throws RemoteException, AlreadyBoundException, InterruptedException{
        try {
            ...
            Registry registry;
            try {
                registry = LocateRegistry.createRegistry(VSServerConfig.REGISTRY_PORT);
            } catch (RemoteException e) {
                System.out.println("Registry already exists, retrieving existing one...");
                registry = LocateRegistry.getRegistry(VSServerConfig.REGISTRY_PORT);
            }
            registry.bind("auctionService", stub);
            System.out.println("Start VSAuctionServer on port " + VSServerConfig.REGISTRY_PORT);
