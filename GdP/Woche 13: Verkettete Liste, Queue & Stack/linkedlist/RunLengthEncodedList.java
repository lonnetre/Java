...
public class RunLengthEncodedList_L implements CharList_L {

    private RunLengthEncodedNode_L head;
    private long size;
    ...
    public void append(char symbol) {
        ...
        if (this.size() == 0) {
            this.head = new RunLengthEncodedNode_L(symbol);
            this.size = 1;
            return;
        }
        ...
        if (current.getSymbol() == symbol) {
            ...
            current.setNext(new RunLengthEncodedNode_L(symbol));
        }
    ...
    public String toString() {
        ...
        while(current != null) {
            ...
            String symbol = Character.toString(current.getSymbol());
    ...
    @Override
    public void insert(char symbol, long index) {
        ...
        if (index == 0 && symbol != this.head.getSymbol()) {
            // Special case: Adds a new node to the beginning of the list
            RunLengthEncodedNode_L newSymbolNode = new RunLengthEncodedNode_L(symbol);
        ...
        while(current != null) {
            ...
            if ((indexContainedByCurrentNode || indexDirectlyAfterCurrentNode) && symbol == current.getSymbol()) {
                // The symbol adds to the current node
                current.increment();
                break;

            } else if (indexDirectlyAfterCurrentNode) {
                if (symbol == current.getNext().getSymbol()) {
                ...
                RunLengthEncodedNode_L nextNode = new RunLengthEncodedNode_L(current.getSymbol());
    ...
    public char remove(long index) {
        ...
        if (index < this.head.getAmount()) {
            char result = this.head.getSymbol();
            this.head.decrement();
        ...
        RunLengthEncodedNode_L previous = this.head;
        RunLengthEncodedNode_L current = this.head.getNext();
        ...
        while(current != null) {
            ...
            if (indexContainedByCurrentNode) {
                ...
                if (current.getAmount() == 0) {
                    ...
                    RunLengthEncodedNode_L next = previous.getNext();
